    ...
    def watch_video(self, title):
        ...
        for video in self.videos:
            if video.title == title:
                ...
                while video.time_now < video.duration + 1:
                    print(f"Смотрим {video.title}: {video.time_now} сек.")
                    time.sleep(1)  # Имитация просмотра
                    video.time_now += 1
...
ur = UrTube()
v1 = Video('Лучший язык программирования 2024 года', 200, 15)
v2 = Video('Для чего девушкам парень программист?', 10, 10, adult_mode=True)
# Добавление видео
ur.add(v1, v2)
# Проверка поиска
